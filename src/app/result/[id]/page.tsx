...
import { RetroPortal } from '@/components/result/RetroPortal';
...
export default function ResultPage() {
    ...
    // Mock content for ELON verification to avoid API costs
    const ELON_MOCK_REPORT = `
# VISIONARY AUDIT: CONSCIOUSNESS TYPE E-1

## 1. NEURAL ARCHITECTURE
Your voice patterns exhibit **Planetary Resonance**. The pitch variance suggests a mind that operates on multiple timelines simultaneously. Standard human constraints (e.g., "sleep," "linear time") appear to be bypassed.

## 2. STRATEGIC POSITIONING
*   **Mars Readiness**: 98.4%
*   **Meme-Energy Density**: CRITICAL
*   **Regulatory Friction Tolerance**: ZERO

## 3. IDENTIFIED PROTOCOLS
You are currently optimized for **Hard-Core Engineering** and **Rapid Iteration**. Avoid meetings at all costs. If a meeting must occur, ensure it is 100% focused on technical truth.

---
*Verified for Parody Research purposes (Non-Generative Static Asset)*
`;
    ...
    const handleCheckout = async (type: 'solo' | 'couple' | 'spy' | 'identity_solo' | 'identity_duo') => {
        // SAFETY: Strictly prevent ELON mode from triggering any payments
        if (isElonMode) {
            console.error("Payment blocked: ELON mode is strictly non-monetized.");
            alert("This feature is for parody only and currently non-monetized.");
            return;
        }
    ...
    // SAFETY: ELON results are always "premium" if verified or if they have analysis, but strictly non-monetized
    const isPremium = result.isPremium === true || isDevMode || (isElonMode && isVerifyMode);
    ...
    return (
        <main className={`relative bg-black text-white selection:bg-cyan-500/30 font-sans overflow-x-hidden w-full ${isElonMode ? 'h-screen overflow-hidden' : ''}`}>
            {isElonMode ? (
                <div className="relative w-full h-full overflow-auto">
                    <RetroPortal
                        ...
                        onDownloadCard={() => {
                            ...
                            if (btn) btn.click();
                            else alert("Neural Print initializing... Please try again and wait for the counter to stabilize.");
                        }}
                    />

                    {/* Hidden/Off-screen Card Generator for Export logic */}
                    <div className="fixed -left-[2000px] pointer-events-none opacity-0">
                        <SoloIdentityCard
                            mbti={result.mbti as MBTIType || 'INTJ'}
                            voiceTypeCode={result.typeCode as TypeCode}
                ...
                <>
                    ...
                    <div className="h-[400vh] w-full">
                        ...
                        <motion.section
                            ...
                            className={`fixed inset-0 flex flex-col items-center justify-center z-10 pointer-events-none ${isElonMode ? 'bg-[#000080]' : ''}`}
                        >
                            <div className={`text-[120px] mb-12 animate-pulse-slow grayscale opacity-40 ${isElonMode ? 'animate-bounce !grayscale-0' : ''}`}>
                                {voiceType.icon}
                            </div>
                            <h2 className={`text-3xl md:text-9xl font-black uppercase text-white mb-6 tracking-[-0.05em] leading-none opacity-20 md:opacity-80 px-8 text-center ${isElonMode ? '!text-yellow-400 !font-serif italic' : ''}`}>
                                {voiceType.name}
                            </h2>
                            ...
                            {isElonMode && (
                                <div className="mt-8 text-magenta-500 font-bold animate-pulse text-xs tracking-widest">
                                    [ PLANETARY ENTROPY DETECTED ]
                                </div>
                            )}
                        ...
                        <div className="relative z-30 pt-[250vh] pb-32 flex flex-col items-center bg-transparent">
                            <div className="w-full max-w-2xl px-4 flex flex-col items-center">
                                ...
                                {/* ELON Mode Parody Disclaimer */}
                                {isElonMode && (
                                    <section className="w-full py-24 mb-12">
                                        <div className="glass rounded-3xl p-8 border border-red-500/20 bg-red-500/5 text-center">
                                            <div className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em] mb-4">Parody Advisory</div>
                                            <p className="text-xs text-gray-400 leading-relaxed font-mono uppercase tracking-widest max-w-lg mx-auto italic">
                                                This diagnostic is an AI research experiment and entertainment parody. VoiceGlow is not affiliated with, endorsed by, or representing any specific individuals. All content produced is generative fiction for entertainment purposes only and provided free of charge.
                                            </p>
                                        </div>
                                    </section>
                                )}
                                ...
                                {/* ELON MOCK VERIFICATION (ADMIN ONLY) */}
                                {isElonMode && isVerifyMode && (
                                    <section className="w-full flex flex-col items-center py-64 border-t border-white/5">
                                        <div className="w-full max-w-2xl">
                                            <div className="text-center mb-32 opacity-80">
                                                <h2 className="text-xl font-black text-white uppercase tracking-[0.6em] italic">Visionary Intelligence Audit</h2>
                                                <p className="text-[9px] font-mono text-cyan-400 mt-4 uppercase tracking-[0.4em]">Verified Parody Mode (Free)</p>
                                            </div>
                                            <div className="prose prose-invert prose-sm max-w-none text-left leading-loose opacity-80">
                                                <ReactMarkdown
                                                    components={{
